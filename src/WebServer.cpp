...
void
WebServer::init(int port,std::string user, std::string password, std::string database_name, int log_write, int opt_liner, int trig_mode,
...
void WebServer::thread_pool() {
    ...
    //线程池
    m_pool = new threadPool<http_conn>(m_actor_model, m_conn_pool, m_thread_num);
...
void WebServer::sql_pool() {
    ...
    //初始化数据库读取表
    users->initmysql_result(m_conn_pool);
}
...
void WebServer::eventListen() {
    ...
    utils.addsig(SIGALRM, utils.sig_handlder, false);
    utils.addsig(SIGTERM, utils.sig_handlder, false);

    alarm(TiMESLOT);

    //工具类，信号和描述符基础操作
    Utls::u_pipfd = m_pipefd;
    Utls::u_epollfd = m_epollfd;
}
...
void WebServer::timer(int connfd, struct sockaddr_in client_address) {
    ...
    users[connfd].init(connfd, client_address, m_root, m_CONNTrigmode, m_close_log, m_user, m_password, m_database_name);
...
void WebServer::deal_timer(util_timer *timer, int sockfd) {
    ...
    if(timer)
    {
        utils.m_timer_lst.del_tiemer(timer);
    }
